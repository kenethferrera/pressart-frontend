...
testPaths.forEach(imagePath => {
  ...
  if (imagePath.startsWith('paintings/')) {
    ...
    const number = imagePath.replace('among-us/among_', '').padStart(2, '0');
    localPath = `Among Us/AMONG_${number}`;
    ...
    const number = imagePath.replace('collage/colagem_', '').padStart(2, '0');
    localPath = `Collage/COLAGEM_${number}`;
