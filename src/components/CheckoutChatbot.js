...
const CheckoutChatbot = () => {
  ...
  const findImageByCode = (code) => {
    ...
    console.log('🔍 findImageByCode called with:', code);
    
    const parts = code.split('-');
    const itemNumber = parseInt(parts[parts.length - 1]);
    const categoryKey = parts.slice(0, -1).join('-');
    
    console.log('🔍 Parsed parts:', { parts, itemNumber, categoryKey });
    ...
    console.log('🔍 Category mapping:', { categoryKey, categoryId });
    ...
    const categoryImages = generateImagePaths(categoryId);
    console.log('🔍 Generated images count:', categoryImages.length);
    
    // Get the image at the specified index (itemNumber - 1 since arrays are 0-based)
    const imageIndex = itemNumber - 1;
    console.log('🔍 Image index calculation:', { itemNumber, imageIndex });
    
    if (imageIndex < 0 || imageIndex >= categoryImages.length) {
      console.log('🔍 Index out of range:', { imageIndex, length: categoryImages.length });
      return null;
    }
